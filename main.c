...
void control_player(Sprite *sprites)
{
  ...
  {
    if (sprites[PLAYER1].dstrect.y == WINDOW_HEIGHT - ANIMATION_FRAME_HEIGHT)
    {
  ...
  {
    if (sprites[PLAYER2].dstrect.y == WINDOW_HEIGHT - ANIMATION_FRAME_HEIGHT)
    {
...
void control_oponent(Sprite *sprites, unsigned int is_npc)
{
  ...
  else
  {
    ...
    if (sprites[BALL].dstrect.y >  max_jump && sprites[npc].dstrect.y == WINDOW_HEIGHT - ANIMATION_FRAME_HEIGHT)
    {
...
{
  if (ball->dstrect.y >= WINDOW_HEIGHT - ball->dstrect.h)
  {
  ...
  if (ball->dstrect.x >= WINDOW_WIDTH - ball->dstrect.w)
  {
